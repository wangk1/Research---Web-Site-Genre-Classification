...
from db.db_collections.mongo_collections import URLQueue
import util.base_util as base_util
import util.evolution as repair
...
def start_bow():
    ...
    queue.decrement()
    url_obj_iterable=URLQueue().iter("genre_bow")
    ...
    for i in range(queue.get(),total_urls):

        try:
            print('New url {}'.format(i))

            url_obj=next(url_obj_iterable).document
            ...
            if not hasattr(url_obj,'page'):
                raise Exception('url {} No has page'.format(i))
            ...
            if not url_to_bow.select(url=url_obj["url"]).find_one():
                ...
                print('Exists bow url number {}'.format(i))

        except Exception as ex:
            Logger.error(url_obj['url']+":::"+str(ex),"C:/Users/Kevin/Desktop/GitHub/Research/Webscraper/bad_url_summarize_bow.txt")
